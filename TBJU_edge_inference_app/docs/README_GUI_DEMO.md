...
1. 点击左侧"打开摄像头"
2. 默认使用 `/dev/video21`（USB 机载摄像头），Windows 使用设备 0
...
PX4 飞控通过 MAVLink 协议连接，实时显示姿态、高度、距离、GPS、电池等信息。

1. 安装依赖：`pip install pymavlink`
2. 接线：PX4 Telem TX → ELF2 UART4_RX，PX4 Telem RX → ELF2 UART4_TX，共地
3. 启动 GUI，飞控面板自动连接（飞控需先上电）
4. 异物检测时，飞控快照自动存储到事件中

环境变量：
- `TBJU_DISABLE_FLIGHT=1` — 禁用飞控模块
- `TBJU_FLIGHT_PORT=/dev/ttyS4` — 串口设备
- `TBJU_FLIGHT_BAUD=57600` — 波特率

CLI 测试：
```bash
python scripts/mavlink_test.py --port /dev/ttyS4 --baudrate 57600
```
...
# 摄像头
python scripts/inference_tbju_stream.py --camera /dev/video21 --show --fps 30 --every_n 2 --all_cores
